...
#define DATA_BITS ((1 << PD2))  // only one data bit on PD2 
#define ENABLE_BIT (1 << PD4)    // Enable pin on PD4
...
void lcd_init(void)
{
    ...
    DDRD |= DATA_BITS;                  // Set the DDR register bits for ports B and D
         ...
         // Set the DDR register bits for PD7-PD4
...
void lcd_writenibble(unsigned char lcdbits)
{
  ...
  PORTB |= (1 << PD4);        // Set E to 1
  PORTB |= (1 << PD4);        // Make E longer
  PORTB &= ~(1 << PD4);       // Set E to 0
}
